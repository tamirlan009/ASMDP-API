...
class Detection:
    def __init__(self, weightsPath, cfgPath, objNames, frameSize, date):
        ...
        if count_detection > 0:
            ...
            if len(detect) != 0:
                ...
                for d in tracker:
                    ...
                    cv.rectangle(frame, ((int)(d[0]), (int)(d[1])), ((int)(d[2]), (int)(d[3])), self.color_list[(int)(d[4])], 2)
    ...
    def img_save(self, frame, count):
        ...
        with open(f"{file_name}",'w') as file:
            file.write('hi')
            file.close()
        self.pothole_db.append((db_img_name,text_file_db, 43.121212, 54.1212212, count))
    ...
    def run(self, file, data_table):
        ...
        try:
            ...
            data_table.count_pothole = self.object_detected
            data_table.count_image = self.save_img_count
            data_table.url = self.url
            data_table.send_detect = False
            data_table.slug = self.slugify(data_table.date)
            data_table.save()
